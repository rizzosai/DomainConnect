...
@app.route('/claim-domain')
def claim_domain_page():
    return send_from_directory('static', 'claim-domain.html')
    ...
    try:
        ...
        if event['type'] == 'checkout.session.completed':
            ...
            if domain_rental:
                ...
                hold_result = opensrs.hold_domain(domain_rental.domain_name)
...
def check_domain_availability():
    try:
        ...
        opensrs = OpenSRSClient()
        check_result = opensrs.check_domain_availability(domain)
...
def process_domain_payment():
    try:
        ...
        registration_result = opensrs.register_domain(domain_name, email, full_name)
